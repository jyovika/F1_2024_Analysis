...
Below, I show the 2011 analysis as an example (converting milliseconds → seconds, computing positionGain = grid − positionOrder, then running Pearson’s rcorr() on the three variables seconds, positionOrder, and positionGain). I repeat the exact same steps for 2015, 2019, and 2024.
...
rcorr(as.matrix(data_2011), type = "spearman")
...
rcorr(as.matrix(data_2015), type = "spearman")
...
rcorr(as.matrix(data_2019), type = "spearman")
...
rcorr(as.matrix(data_2024), type = "spearman")
```
...
6. Harrell Jr F (2025). _Hmisc: Harrell Miscellaneous_. R package version 5.2-3,
