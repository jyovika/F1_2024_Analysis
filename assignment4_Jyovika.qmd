...
Below, I show the 2011 analysis as an example (converting milliseconds → seconds, computing positionGain = grid − positionOrder, then running Spearman’s rcorr() on the three variables seconds, positionOrder, and positionGain because the Spearman rank‐order correlation coefficient is robust to the skewed distribution of pit‐stop times). I repeat the exact same steps for 2015, 2019, and 2024.
...
data_2011 <- as.data.frame(data_2011)

corr_2011 <- rcorr(as.matrix(data_2011), type = "spearman")
...
corr_2015 <- rcorr(as.matrix(data_2015), type = "spearman")
...
corr_2019 <- rcorr(as.matrix(data_2019), type = "spearman")
...
corr_2024 <- rcorr(as.matrix(data_2024), type = "spearman")
```

Visualisation 

```{r}
# 1. Custom upper‐panel function that indexes columns by name
upper_fn <- function(data, mapping, ...) {
  # Extract the column names as strings
  x_name <- as_label(mapping$x)
  y_name <- as_label(mapping$y)
  
  # Pull out the numeric vectors directly
  x <- data[[x_name]]
  y <- data[[y_name]]
  
  # Run Spearman correlation test
  corr_res <- cor.test(x, y, method = "spearman", exact = FALSE)
  rho  <- round(corr_res$estimate, 2)
  pval <- round(corr_res$p.value,   3)
  
  label <- paste0("ρ = ", rho, "\n(p = ", pval, ")")
  
  # Create a blank ggplot and annotate at the center
  ggplot() +
    annotate(
      "text",
...
rhos_2011 <- list(
  rho_finish = corr_2011$r["seconds", "positionOrder"],
  p_finish   = corr_2011$P["seconds", "positionOrder"],
  rho_gain   = corr_2011$r["seconds", "positionGain"],
  p_gain     = corr_2011$P["seconds", "positionGain"]
)

ggpairs(
  data_2011,
  lower = list(continuous = "smooth"),       # scatter + LOESS in lower triangle
  diag  = list(continuous = "densityDiag"),   # density plots on diagonal
  upper = list(continuous = wrap(upper_fn))   # our custom Spearman ρ + p in upper
) + ggtitle("2011: PitStop ↔ Finish / Gain")
```


```{r, echo=FALSE}
rhos_2015 <- list(
  rho_finish = corr_2015$r["seconds", "positionOrder"],
  p_finish   = corr_2015$P["seconds", "positionOrder"],
  rho_gain   = corr_2015$r["seconds", "positionGain"],
  p_gain     = corr_2015$P["seconds", "positionGain"]
)

rhos_2019 <- list(
  rho_finish = corr_2019$r["seconds", "positionOrder"],
  p_finish   = corr_2019$P["seconds", "positionOrder"],
  rho_gain   = corr_2019$r["seconds", "positionGain"],
  p_gain     = corr_2019$P["seconds", "positionGain"]
)

rhos_2024 <- list(
  rho_finish = corr_2024$r["seconds", "positionOrder"],
  p_finish   = corr_2024$P["seconds", "positionOrder"],
  rho_gain   = corr_2024$r["seconds", "positionGain"],
  p_gain     = corr_2024$P["seconds", "positionGain"]
)
```
...
9. Harrell Jr F (2025). _Hmisc: Harrell Miscellaneous_. R package version 5.2-3,
  <https://CRAN.R-project.org/package=Hmisc>.
  
10. Wickham H, Averick M, Bryan J, Chang W, McGowan LD, François R, Grolemund G, Hayes A,
